...
object decode_record(string rbuff) {
  [srcaddr,dstaddr,nexthop,input,output,dPkts,dOctets,first,last,srcport,dstport,tcp_flags,prot,tos,src_as,dst_as,src_mask,dst_mask] = array_sscanf(rbuff, "%4c%4c%4c%2c%2c%4c%4c%2c%2c%*c%c%c%c%2c%2c:%c:%c%2*c");
  return this_object();
}
